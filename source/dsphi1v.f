...
*   (26-Apr-2023):
*   1) Copied from dsphiv.f (Ver. 0.1), and revised for the function
*      DGPHI1V to calculate t*phi(tA)u, not exp(t*A)v + t*phi(tA)u.
*   2) Revised to replace DDOT with commands.
*   3) Revised to replace DAXPY with commands in a loop.
*   4) Commented out a line which outputs 'happy breakdown'.
******************************
*----------------------------------------------------------------------|
      subroutine DSPHI1V( n, m, t, u, w, tol, anorm,
...
*-----Purpose----------------------------------------------------------|
*
*---  DSPHIV computes w = t*phi1(tA)u, where
*     phi1(z) = (exp(z)-1)/z and A is a Symmetric matrix.
*
...
*!      if ( m.ge.n .or. m.le.0 ) iflag = -3
      if ( iflag.ne.0 ) stop 'bad sizes (in input of DSPHI1V)'
*
*---  initialisations ...
*
      k1 = 3
...
*
 ...
 401  continue
*
*---  compute w = beta*t_step*V*phi1(t_step*H)*e1
*
      nexph = nexph + 1
